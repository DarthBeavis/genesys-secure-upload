...
const CONFIG = {
  // Trigger phrase — must match what the agent pastes into the chat
  TRIGGER_PHRASE:    'please upload',

  // Delay (ms) before modal auto-opens after trigger detected
  AUTO_OPEN_DELAY:   2500,

  // Your malware scan API
  SCAN_ENDPOINT:     'https://your-scan-service.example.com/api/scan', // ← REPLACE
  SCAN_API_KEY:      'YOUR_SCAN_API_KEY',                               // ← REPLACE
      ...
      <div class="gcsu-panel">
        ...
        <div class="gcsu-body">
          ...
          <div class="gcsu-steps">
            ...
            <div class="gcsu-st active" id="gcsu-st2">
              <div class="gcsu-dot">2</div>
              <div class="gcsu-lbl">Scan</div>
            </div>
            <div class="gcsu-line" id="gcsu-ln2"></div>
            <div class="gcsu-st" id="gcsu-st3">
              <div class="gcsu-dot">3</div>
              <div class="gcsu-lbl">Send</div>
            </div>
          </div>
          ...
          <button id="gcsu-action" disabled>
            ...
            <span id="gcsu-btn-lbl">Scan &amp; Send to Agent</span>
          </button>

          <div class="gcsu-footer">
            <span>ℹ️</span>
            Files are scanned for malware before delivery. Only clean files reach your agent.
          </div>
        </div>
      </div>
    </div>
...
function initUpload() {
  ...
  const st3 = document.getElementById('gcsu-st3');
  const ln1 = document.getElementById('gcsu-ln1');
  const ln2 = document.getElementById('gcsu-ln2');
  ...
  function setSteps(active, errAt) {
    ...
    ln2.className = 'gcsu-line' + (active>2&&!errAt?' done':'');
  }
  ...
  async function scanFile(file) {
    setSteps(2); setProgress('Scanning for threats…', 5, 'scan');
    const form = new FormData();
    form.append('file', file, file.name);
    let tick = 5;
    const ticker = setInterval(() => { tick = Math.min(tick+7,85); setProgress('Scanning…', tick, 'scan'); }, 300);
    try {
      const res = await fetch(CONFIG.SCAN_ENDPOINT, {
        method: 'POST', headers: { 'X-API-Key': CONFIG.SCAN_API_KEY }, body: form
      });
      clearInterval(ticker);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      setProgress('Scan complete', 100, 'scan');
      return await res.json();
    } catch(err) { clearInterval(ticker); throw new Error('Scan failed: '+err.message); }
  }

  // Genesys upload
  async function uploadToGenesys(file) {
    setSteps(3); setProgress('Uploading to Genesys…', 10, 'upload');
    return new Promise((resolve, reject) => {
      if (typeof Genesys !== 'function') { reject(new Error('Genesys SDK not found')); return; }
      let tick = 10;
      const ticker = setInterval(() => { tick = Math.min(tick+10,75); setProgress('Uploading…', tick, 'upload'); }, 250);
      Genesys('command', 'MessagingService.requestUpload', { file },
        async details => {
          clearInterval(ticker);
          const url  = details.uploadUrl || details.url;
          const hdrs = details.headers || {};
          if (!url) { reject(new Error('No presigned URL from Genesys: '+JSON.stringify(details))); return; }
          try {
            setProgress('Uploading…', 85, 'upload');
        ...
        err => { clearInterval(ticker); reject(new Error('Genesys error: '+JSON.stringify(err))); }
      );
    });
  }
  ...
  actionBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!scanResult.clean) throw {
        stage:'threat',
        msg: 'Blocked by scan.' + (scanResult.threat ? ' Threat: '+scanResult.threat : '')
      };

      try { await uploadToGenesys(file); }
      catch(e) { throw { stage:'upload', msg: e.message }; }

      notifyAgent(file);

      st3.className = 'gcsu-st done';
      ln2.className = 'gcsu-line done';
      showStatus(`"${file.name}" scanned and sent to your agent.`, 'success');
      ...
      const msgs = {
        ...
        scan:   ['Scan unavailable: '+err.msg, 'warning'],
        upload: ['Upload failed: '+err.msg, 'error']
      };
      const [txt, type] = msgs[err.stage] || [String(err), 'error'];
      showStatus(txt, type);
